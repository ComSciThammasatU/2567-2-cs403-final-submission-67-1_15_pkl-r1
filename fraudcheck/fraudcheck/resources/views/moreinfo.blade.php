...
<body>
    ...
    <div class="container my-5">
                ...
                <table class="table table-striped">
                    <tbody>
                        <tr>
                            ...
                            <td>{{ $report->user_id }}</td> <!--หมายเลขผู้เพิ่ม-->
                        </tr>
                        <tr>
                            <th scope="row">หมายเลขรายงาน :</th>
                            <td>{{ $report->id }}</td> <!--หมายเลขรายงาน-->
                        </tr>
                        <tr>
                            <th scope="row">ชื่อ :</th> 
                            <td>{{ NamecensorField($report->name) }}</td> <!--เซนเซอร์ชื่อ-->
                        </tr>
                        <tr>
                            <th scope="row">นามสกุล :</th>
                            <td>{{ NamecensorField($report->surname) }}</td> <!--เซนเซอร์นามสกุล-->
                        </tr> 
                        <tr>
                            <th scope="row">เลขบัตรประชาชน :</th>
                            <td>{{ censorField($report->idcard) }}</td> <!--เซนเซอร์เลขบัตรประชาชน-->
                        </tr>
                        <tr>
                            <th scope="row">Promtpay,Truemoney :</th>
                            <td>{{ censorField($report->telephone) }}</td> <!--เซนเซอร์พร้อมเพลย์-->
                        </tr>
                        <tr>
                            <th scope="row">ธนาคาร :</th>
                            <td>{{ $report->bankType ?? '-' }}</td> <!--ดึงชนิดธนาคาร ถ้าไม่มีให้เป็นค่าว่าง-->
                        </tr>
                        <tr>
                            <th scope="row">เลขบัญชี :</th>
                            <td>{{ censorField($report->bankAccount) }}</td> <!--เซนเซอร์เลขธนาคาร-->
                        </tr>
                        <tr>
                            <th scope="row">จำนวนเงิน :</th>
                            <td>{{ $report->amount}}</td> <!--จำนวนเงิน-->
                        </tr>
                        <tr>
                            <th scope="row">สินค้า :</th>
                            <td>{{ $report->productName }}</td> <!--สินค้า-->
                        </tr>
                        <tr>
                            <th scope="row">แอปพลิเคชัน :</th>
                            <td>{{ $report->communication_platform }}</td> <!--แพลตฟอร์มที่โอน-->
                        </tr>
                        <tr>
                            <th scope="row">วันโอนเงิน :</th>
                            <td>{{ $report->payment_date->format('d-m-Y') }}</td> <!--วันโอนเงิน-->
                        </tr>
                        <tr>
                            <th scope="row">วันตั้งโพส :</th>
                            <td>{{ $report->created_at->format('d-m-Y') }}</td> <!--วันเริ่มสร้างรายงาน-->
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        @if(!empty($report->product_images)) <!--ตรวจสอบว่าภาพไม่ว่างเปล่า ให้ทำเงื่อนไขข้างใน-->
        <div class="card mb-4">
            <div class="card-body">
                <h5 class="card-title">ภาพสินค้า :</h5>
                <div class="image-gallery">
                    @foreach($report->product_images as $image) <!--วนลูปภาพ-->
                        <img src="{{ $image }}" alt="ภาพสินค้า" class="img-fluid"> <!--แสดงภาพสินค้าที่เก็บอยู่ในคอลัมธ์ product_image-->
                    @endforeach
                </div>
            </div>
        </div>

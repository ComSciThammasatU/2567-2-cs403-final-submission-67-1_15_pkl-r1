        ...
        <div class="row justify-content-center">
            <div class="col-md-10">
                ...
                @foreach($protests as $protest)
                    <div class="card mb-4">
                        <div class="card-body">
                            <h5 class="card-title">หัวข้อ: {{ $protest->title }}</h5> <!--แสดงหัวข้อจากฐานข้อมูล-->
                            <p class="card-text">หมายเลขผู้แจ้ง: {{ $protest->user_id }}</p> <!--แสดงเลขผู้แจ้งจากฐานข้อมูล-->
                            <p class="card-text">หมายเลขรายงาน: {{ $protest->report_id }}</p> <!--แสดงหมายเลขรายงานจากฐานข้อมูล-->
                            <p class="card-text">หมายเหตุ: {{ $protest->description }}</p> <!--แสดงอีเมลที่ผู้ใช้เก็บจากฐานข้อมูล-->
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <small class="text-muted">ชื่อผู้แจ้ง: {{ $protest->user->username ?? 'ไม่ระบุ' }}</small> <!--แสดงชื่อผู้แจ้งจากฐานข้อมูล App_User โดยชี้ไปที่ฟังก์ชัน user ในโมเดล Protests-->
                                <small class="text-muted">วันที่แจ้ง: {{ $protest->created_at->format('d/m/Y H:i') }}</small> <!--แสดงวันเวลาแจ้งรายงาน-->
                            </div>
                            
                            @if($protest->images) <!--แสดงรูปภาพ-->
                                <div class="row mt-3">
                                    @foreach($protest->images as $index => $image) <!--เก็บตำแหน่ง 0 ภาพที่ 1, ตำแหน่ง 1 ภาพที่ 2, ....-->
                                        <div class="col-md-4 col-6 mb-3">
                                            <!--แสดงภาพ, ใช้จาวาสคริปเพื่อขยายภาพให้ใหญ่-->
                                            <img src="{{ $image }}" alt="Protest Image {{ $index + 1 }}"  
                                                 class="protest-image"
                                                 data-bs-toggle="modal" 
                                                 data-bs-target="#imageModal" 
                                                 data-bs-image="{{ $image }}"> <!--ส่งภาพจากฐานข้อมูล ให้ javascript-->
                                        </div>
                                    @endforeach
                                </div>
                            @endif
                            ...
                            <div class="form-check form-switch position-absolute top-0 end-0 m-2">
                                <input class="form-check-input" type="checkbox" role="switch" id="flexSwitchCheckDefault{{ $protest->id }}"
                                    data-protest-id="{{ $protest->id }}" onchange="updateProtestStatus({{ $protest->id }}, this.checked)">
                                <label class="form-check-label protest-status text-warning" for="flexSwitchCheckDefault{{ $protest->id }}">ยังไม่ดำเนินการ</label>
                            </div>

                        ...
                        <tbody>
                            @foreach ($reports as $report)
                                ...
                                <tr>
                                    ...
                                    <td>{{ NamecensorField($report->name) }}</td>
                                    <td>{{ NamecensorField($report->surname) }}</td>
                                    <td>{{ censorField($report->idcard) }}</td>
                                    <td>{{ censorField($report->telephone) }}</td>
                                    <td>{{ censorField($report->bankAccount) }}</td>
                                    ...
                                    <td> <!--วนลูป 3 คอลัมธ์-->
                                        <div class="d-flex flex-wrap">
                                            @foreach (['product_images', 'conversation_images', 'payment_proof_images'] as $imageType)
                                            <!--ถ้ามีรูปภาพ ทำเงื่อนไขล่าง-->
                                            @if (!empty($report->$imageType))
                                                @foreach ($report->$imageType as $index => $image)
                                                    <img src="{{ $image }}" class="image-thumbnail"
                                                        data-bs-toggle="modal" data-bs-target="#imageModal"
                                                        data-image-src="{{ $image }}"
                                                        alt="{{ $imageType }} {{ $index + 1 }}"
                                                        title="{{ ucfirst(str_replace('_', ' ', $imageType)) }}">
                                                @endforeach
                                            @endif
                                        @endforeach
                                        </div>
                                  ...
                                  <!-- กล่องควบคุมสถานะรายงานแต่ละรายการ (ใช้กับ JavaScript) -->
                                    <div class="status-controls" id="status-controls-{{ $report->id }}">
                                        ...
                                        @if ($report->status !== 'rejected')
                                            ...
                                            <div class="form-check mb-2">
                                                ...
                                                <input 
                                                    ...
                                                    type="checkbox"
                                                    id="status-{{ $report->id }}" {{-- เชื่อมกับ label ด้านล่าง --}}
                                                    data-report-id="{{ $report->id }}" {{-- ใช้ใน JavaScript --}}
                                                    {{ $report->status === 'approve' ? 'checked' : '' }} {{-- ถ้าอนุมัติแล้วให้ติ๊กไว้ --}}
                                                >

                                                <!-- Label ที่แสดงข้อความสถานะ (รอดำเนินการ / อนุมัติแล้ว) -->
                                                <label class="form-check-label" for="status-{{ $report->id }}">
                                                    <span class="status-text">
                                                        {{ $report->status === 'approve' ? 'อนุมัติแล้ว' : 'รอดำเนินการ' }}
                                                    </span>
                                                </label>
                                    ...
                                    <button class="btn btn-reject {{ $report->status === 'rejected' ? 'disabled' : '' }}
                                        "data-report-id="{{ $report->id }}"
                                        {{ $report->status === 'rejected' ? 'disabled' : '' }}><!-- ปิดปุ่มปฏิเสธ-->
                                    ...
                                    <!--เพิ่มแจ้งเตือนเล็กๆ ให้ฟอร์มค้นหา DOM หาเจอ-->
                                        ...
                                        <td>
                                            <!--ปุ่มลบรายงาน-->
                                            <button class="btn-delete" data-report-id="{{ $report->id }}">

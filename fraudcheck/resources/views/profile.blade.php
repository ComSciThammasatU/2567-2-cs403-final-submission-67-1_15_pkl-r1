            ...
            <div class="col-md-8">
                <div class="card mb-4">
                    <div class="card-body text-center">
                        ...
                        <img src="{{ Auth::user()->profile_picture ? asset('storage/profile_pictures/'.Auth::user()->profile_picture) : 'https://via.placeholder.com/150' }}"
                         alt="Profile Picture" class="profile-picture mb-4">
                        <h3 class="card-title mb-3">{{ Auth::user()->username }}</h3> <!--แสดง username --> 
                        <p class="card-text"><strong>ID:</strong> {{ Auth::user()->id }}</p>  <!--แสดงเลข ID --> 
                        <p class="card-text"><strong>อีเมล:</strong> {{ Auth::user()->email }}</p> <!--แสดง email --> 
                        <p class="card-text"><strong>สถานะ:</strong> {{ Auth::user()->role }}</p> <!--แสดง สถานะ --> 
                    </div>
                </div>
                <div class="d-grid gap-3">
                    
                    <!--Checkroll ว่าเป็น ผู้ใช้งาน หรือ แอดมิน--> 
                    @if(Auth::user()->role !== 'admin') <!--ผู้ใช้งาน -->
                        <a href="{{ route('user.reports') }}" class="btn btn-custom btn-main">ดูรายงาน</a>
                    @endif
                    <a href="/search" class="btn btn-custom btn-main">ค้นหารายงาน</a> <!--ปุ่มนี้มีทั้ง ผู้ใช้งาน และ แอดมิน--> 
                    @if(Auth::user()->role === 'admin') <!--แอดมิน -->
                        <a href="/reportlist" class="btn btn-custom btn-main">ดูการส่งรายงาน</a>
                        <a href="{{ route('protests.index') }}" class="btn btn-custom btn-main">ดูการแจ้งลบรายงาน</a>
                    @endif

                    <!--ผู้ใช้งาน และ แอดมิน-->
                    <a href="{{ url('/editprofile') }}" class="btn btn-custom btn-secondary-custom">แก้ไขข้อมูลส่วนตัว</a>
                    <a href="{{ route('editpassword') }}" class="btn btn-custom btn-secondary-custom">เปลี่ยนรหัสผ่าน</a>
